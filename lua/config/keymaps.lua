local set = vim.keymap.set
local harpoon = require("harpoon")

-- Misc Leader Actions
set("n", "<leader>/", function()
	vim.cmd.Telescope("live_grep")
end)
set("n", "<leader>?", function()
	vim.cmd.Telescope("keymaps")
end)
set("n", "<leader>ht", function()
	vim.cmd.Telescope("help_tags")
end)
set("v", "<leader>y", '"+y')
set("n", "<leader>m", function()
	vim.cmd.Telescope("man_pages")
end)
set("n", "<leader>a", function()
	vim.cmd.Telescope("autocommands")
end)
-- Buffer Actions
set("n", "<leader>bb", function()
	vim.cmd.Telescope("buffers")
end)
set("n", "<leader>bd", vim.cmd.CloseBuffer)
-- Code / Compile Actions
set("n", "<leader>ca", vim.lsp.buf.code_action)
set("n", "<leader>cs", function()
	vim.cmd.Telescope("treesitter")
end)
set("n", "<leader>cr", vim.lsp.buf.rename)
set("n", "<leader>ch", function()
	vim.cmd.Telescope("command_history")
end)
set("n", "<leader>cc", function()
	vim.cmd.ReadCmd()
end)
set("n", "<leader>cf", vim.lsp.buf.format)
set("n", "<leader>cl", function()
	vim.cmd("che lsp")
end)
-- File Actions
set("n", "<leader>ff", function()
	vim.cmd.Telescope("find_files")
end)
set("n", "<leader>fd", vim.cmd.CreateDir)
set("n", "<leader>fn", vim.cmd.CreateFile)
-- Git Actions
set("n", "<leader>gb", function()
	vim.cmd.Telescope("git_branches")
end)
set("n", "<leader>gg", vim.cmd.Git)
set("n", "<leader>gD", vim.cmd.Gdiffsplit)
set("n", "<leader>gC", vim.cmd.CreateBranch)
set("n", "<leader>gA", function()
	vim.cmd.Git("add --all")
end)
set("n", "<leader>ga", vim.cmd.GitAdd)
set("n", "<leader>gc", function()
	vim.cmd.Git("commit")
end)
set("n", "<leader>gp", function()
	vim.cmd.Git("push")
end)
set("n", "<leader>gP", function()
	vim.cmd.Git("pull --rebase")
end)
set("n", "<leader>gB", function()
	vim.cmd.Git("blame")
end)
set("n", "<leader>gM", function()
	vim.cmd.Git("mergetool")
end)
set("n", "<leader>gd", function()
	vim.cmd.Git("diff")
end)
set("n", "<leader>gl", function()
	vim.cmd.Git("log")
end)
set("n", "<leader>gu", vim.cmd.GitUnstage)
set("n", "<leader>gU", function()
	vim.cmd.Git("restore --staged :/")
end)
set("n", "<leader>gr", vim.cmd.RestoreFile)
set("n", "<leader>gR", vim.cmd.RestoreAllFiles)
set("n", "<leader>ge", vim.cmd.RebaseBranch)
set("n", "<leader>gF", function()
	vim.cmd.Git("fetch")
end)
set("n", "<leader>gj", function()
	vim.cmd("diffget //2<CR>")
end)
set("n", "<leader>gf", function()
	vim.cmd("diffget //2<CR>")
end)
set("n", "<leader>go", vim.cmd.PrintConflicts)
set("n", "<leader>gss", vim.cmd.StashChanges)
set("n", "<leader>gsl", function()
	vim.cmd.Telescope("git_stash")
end)
set("n", "<leader>gsc", function()
	vim.cmd.Git("stash clear")
end)
set("n", "<leader>gmu", vim.cmd.UpdateSubmodule)
-- Harpoon Actions
set("n", "<leader>hh", function()
    harpoon.ui:toggle_quick_menu(harpoon:list())
end)
set("n", "<leader>ha", function ()
    harpoon:list():add()
end)
set("n", "<leader>h1", function ()
    harpoon:list():select(1)
end)
set("n", "<leader>h2", function ()
    harpoon:list():select(2)
end)
set("n", "<leader>h3", function ()
    harpoon:list():select(3)
end)
set("n", "<leader>h4", function ()
    harpoon:list():select(4)
end)
set("n", "<leader>h5", function ()
    harpoon:list():select(5)
end)
set("n", "<leader>h6", function ()
    harpoon:list():select(6)
end)
set("n", "<leader>h7", function ()
    harpoon:list():select(7)
end)
set("n", "<leader>h8", function ()
    harpoon:list():select(8)
end)
set("n", "<leader>h9", function ()
    harpoon:list():select(9)
end)
-- Toggle Actions
set("n", "<leader>tu", vim.cmd.UndotreeToggle)
-- Project Actions
set("n", "<leader>pf", function()
	vim.cmd.Telescope("git_files")
end)
set("n", "<leader>pd", vim.cmd.OpenDir)
set("n", "<leader>pD", function()
	vim.cmd.Telescope("diagnostics")
end)
set("n", "<leader>qq", vim.cmd.Ex)
-- Window Actions
set("n", "<leader>ws", vim.cmd.split)
set("n", "<leader>wv", vim.cmd.vsplit)
set("n", "<leader>wq", vim.cmd.close)
-- Misc Actions
set("n", "gd", function()
	vim.cmd.Telescope("lsp_definitions")
end)
set("n", "gr", function()
	vim.cmd.Telescope("lsp_references")
end)
set("n", "gi", function()
	vim.cmd.Telescope("lsp_implementations")
end)
set("v", "<", "<gv")
set("v", ">", ">gv")
set("n", "<", "<<")
set("n", ">", ">>")
set("v", "p", "P")
set("n", "<C-n>", function()
	vim.cmd("nohlsearch")
end)
set("v", "J", ":m '>+1<CR>gv=gv")
set("v", "K", ":m '<-2<CR>gv=gv")
set("v", "<C-p>", '"*p')
set("n", "K", vim.lsp.buf.hover)
set("n", "J", vim.diagnostic.open_float)
